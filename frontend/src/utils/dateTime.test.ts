...
import { fullCalendarSkopjeDateTimeInput, fullCalendarSkopjeOffsetDateTime } from './dateTime';
...
describe('dateTime FullCalendar helpers', () => {
  it('keeps the visible FullCalendar wall-clock time when creating a Skopje offset timestamp', () => {
    ...
    expect(fullCalendarSkopjeOffsetDateTime(droppedStart)).toBe('2026-06-20T13:30:00+02:00');
    expect(fullCalendarSkopjeOffsetDateTime(droppedEnd)).toBe('2026-06-20T16:00:00+02:00');
  });
  ...
  it('keeps the visible FullCalendar wall-clock time for date-time picker inputs', () => {
    ...
    expect(fullCalendarSkopjeDateTimeInput(selectedSlot)).toBe('2026-06-20T08:00');
  });
});
