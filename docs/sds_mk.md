...
- контролна табла со агрегирани бројачи.
...
Системот по реорганизацијата е модуларен монолит. Сите функционалности се во една Spring Boot апликација, но се раздвоени по функционални пакети: `auth`, `user`, `customer`, `vehicle`, `appointment`, `servicerecord`, `offer`, `document`, `dashboard` и `loyalty`. Заедничката инфраструктура е издвоена во `common`, `config`, `security`, `notification` и `audit`.
...
| Контролна табла | Агрегирани бројачи | `dashboard/controller`, `dashboard/service`, `dashboard/dto` |
...
Поддржувачките потсистеми, како известувања по електронска пошта, генерирање PDF документи, ревизиски записи, контролна табла, глобална обработка на грешки и заедничка пагинација, не се основни доменски модули сами по себе. Тие сепак се значајни за извршување на бизнис процесите и се документирани во поглавјето 7.2.
...
| Интеракции и зависности | Зависи од модулите за клиенти и возила. Објавува настан кон модулот за сервисни документи. Се користи од модулот за лојалност и од контролната табла за агрегирани податоци. |
...
| Состав | `AuthController`, `AdminUserController`, `CustomerController`, `VehicleController`, `AppointmentController`, `ServiceRecordController`, `OfferController`, `ServiceDocumentController`, `DashboardController`. |
...
Клиентската апликација содржи јавни и заштитени рути. Јавни се `login`, `book-appointment`, `appointments/book` и `reservations/cancel/:token`. Заштитените рути се dashboard, customers, vehicles, appointments, services, offers, documents, change password и admin. `RequireAuth` ги штити автентицираните рути, а `RequireRole` го ограничува admin делот.
